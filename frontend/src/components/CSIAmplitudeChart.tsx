...
export function CSIAmplitudeChart({
  ...
  const data = useMemo(() => {
    ...
    return slice.map((h) => {
      const point: Record<string, number | string> = { time: formatTime(h.timestamp) };
      ...
      <div className="chart-wrapper">
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={data} margin={{ top: 8, right: 12, left: 8, bottom: 8 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="var(--grid-stroke, #E5E9F0)" />
            <XAxis
              dataKey="time"
            ...
            <Legend
              ...
              formatter={(value: string) => `SC ${parseInt(value.replace('sc', '')) + 1}`}
            />
